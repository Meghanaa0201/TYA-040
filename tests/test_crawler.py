...
import pytest
from scraper import crawler, core
...
class TestLinkExtraction:
    ...
    def test_extract_absolute_links(self):
        ...
        links = crawler.discover_links(html, "https://example.com")
    ...
    # discover_links explicitly filters anchors starting with #
...
class TestLinkClassification:
    ...
    def test_classify_internal_links(self):
        ...
        '''
        classification = core.classify_links(html, "https://example.com")
...
class TestFileDownload:
    ...
    @patch('scraper.core.polite_delay')
    def test_download_pdf(self, mock_delay):
        ...
        # Mock add_file to prevent file operations error or dependency
        # Mock add_file to prevent file operations error or dependency
        with patch('scraper.storage.add_file'):
            # Use mock_open for file operations
            from unittest.mock import mock_open
            with patch('builtins.open', mock_open()):
                 # Mock os.makedirs to prevent directory creation error
                 with patch('os.makedirs'):
                     # Mock os.path.getsize
                     with patch('os.path.getsize', return_value=123):
                        result = crawler.download_file("https://example.com/doc.pdf", "domain-123", session)

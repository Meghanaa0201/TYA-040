...
def send_immediate_change_notification(recipient: str, change: dict, domain_url: str):
    """Send immediate notification for a single change"""
    if not config.EMAIL_ENABLED:
        return False
        
    page_url = change.get('page_url', 'Unknown URL')
    page_title = change.get('page_title', 'No Title')
    change_type = change.get('change_type', 'unknown').upper()
    
    subject = f"🔔 {change_type}: {page_title}"
    
    similarity_html = ""
    if change.get('similarity_score') is not None:
        similarity_pct = change['similarity_score'] * 100
        similarity_html = f"<p><strong>Similarity:</strong> {similarity_pct:.1f}%</p>"
    ...
    <body>
        <div class="header {change.get('change_type', 'modified')}">
            <h2>{change_type} Page Detected</h2>
            <p><strong>{domain_url}</strong></p>
            <p>Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
        </div>
        ...
        <div class="content">
            ...
            <p>A change was detected on this page during the latest scan.</p>
        </div>
        
        <div style="margin-top: 20px; text-align: center; color: #999; font-size: 0.8em;">
            <p>You are receiving this because you subscribed to immediate alerts.</p>
        </div>
    </body>
    </html>
    """

...
def normalize_url(url):
    ...
    if normalized.endswith('/') and normalized != parsed.scheme + '://' + parsed.netloc.lower() + '/':
        ...
        if domain_id:
            ...
            if existing_page is None:
                ...
                if old_snapshot_path and os.path.exists(old_snapshot_path):
                    ...
                    try:
                        # Find old HTML snapshot
                        old_html_snapshots = sorted([f for f in os.listdir(snapshot_dir) if f.endswith('.html')])
                        old_html_snapshot_path = None
                        if len(old_html_snapshots) >= 2:
                            old_html_snapshot_path = os.path.join(snapshot_dir, old_html_snapshots[-2])
